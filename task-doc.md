...
这里是 2023-2024 年春季学期**软件分析与验证**课程大作业任务说明文档，本次课程大作业的目标是完成一个具有以下功能的程序验证工具：
...
CMinor 语言支持两种 C 中的原子类型 `int`、`float`，支持这两种原子类型的定长的*一维数组*的定义和读写，也支持以这两种原子类型为成员的结构体（`struct`），同时也支持 C 语言中最基本的**顺序**、**条件**（`if-then-else`）和**循环**（`while`、`for` 和 `do-while`）语句，以及函数。
...
- 本次作业的 DDL 是第十八周周五（6 月 28 日）。
  - 对于毕业班同学，DDL 为第十六周周日（6 月 16 日）。
